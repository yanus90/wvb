<?php
add_image_size('slider_image_crop', 1005, 560, true);
add_image_size('post_thumb', 550, 360, true);
add_image_size('post_image', 900, 600, true);
add_image_size('team_image', 1024, 685, true);
add_image_size('team_image_large', 1600, 1070, true);
add_image_size('team_thumb', 660, 442, true);
add_image_size('event_thumb_small', 660, 440, true);
add_image_size('about_wvb_home_image', 1000, 600, true);
add_image_size('commission_image', 850, 550, true);
add_image_size('person_image', 600, 800, true);
add_image_size('sponsor_month_image', 850, 550, true);
add_image_size('product_image_crop', 900, 573, true);
